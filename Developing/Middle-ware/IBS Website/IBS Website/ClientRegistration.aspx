...
<form  method="post" autocomplete="off" id="ClientRegistrationForm" runat="server" target ="_self">
  <div class="container">
	...
	<br>
    <asp:TextBox ID="PasswordCR" runat="server" type="password" placeholder="Password*" pattern = "^(?=.*[a-z])(?=.*[A-Z])(?=.*[/!@#$%^&*_=+-]).{8,}$" title="Password should contain at least one upper character, one lower character and at least 8 numbers" name="psw" required OnTextChanged="PasswordCR_TextChanged" ></asp:TextBox>
	<br>
	...
	<br>
     <asp:RegularExpressionValidator ID="emailVal" ControlToValidate="EmailCR" ErrorMessage="Email format must be valid"  ForeColor="orange" ValidationExpression="^([\w-\.]+)@((\[[0-9]{1,3}\.[0-9]{1,3}\.[0-9]{1,3}\.)|(([\w-]+\.)+))([a-zA-Z]{2,4}|[0-9]{1,3})(\]?)$" runat="server">  </asp:RegularExpressionValidator>  
